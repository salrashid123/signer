...
// todo, figure out how to close the tpm, then reopen it inside the library
//
//	though i'm using the same seed during test initialization and inside the library
//	 i think the tpm is hard reset or not deriving the same owner keys

/*
func TestTPMSignRSAManaged(t *testing.T) {
	tpmDevice, err := simulator.GetWithFixedSeedInsecure(1073741825)
	require.NoError(t, err)
	...
	_, err = tpm2.EvictControl{
		...
		PersistentHandle: tpm2.TPMHandle(0x81008001),
	}.Execute(rwr)
	require.NoError(t, err)
	...
	conf := TPM{
		TpmPath:      "simulator",
		KeyHandle:    tpm2.TPMHandle(0x81008001).HandleValue(),
		PCRs:         []uint{},
		AuthPassword: []byte(""),
	}
